package com.test.flume.sink.filesink;
...
public class FileSink extends AbstractSink implements Configurable {
    ...
    /**
     * Default length of time we wait for blocking BucketWriter calls before timing out the
     * operation. Intended to prevent server hangs.
     */
    ...
    public void configure(Context context) {
        ...
        String directory =
                Preconditions.checkNotNull(context.getString("file.path"), "file.path is required");
        ...
        try {
            ...
            for (txnEventCount = 0; txnEventCount < txnEventMax; txnEventCount++) {
                ...
                if (bucketFileWriter == null) {
                    ...
                    bucketFileWriter.open(realPath, serializerType, serializerContext, rollInterval,
                            timedRollerPool, sfWriters);
